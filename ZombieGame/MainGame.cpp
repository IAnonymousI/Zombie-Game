...
void MainGame::initLevel() {
	...
	for (int i = 0; i < zombiePositions.size(); i++) {
		std::uniform_real_distribution<float> randZSpeed(1.0f, 3.0f);
		_zombies.push_back(new Zombie);
		_zombies.back()->init(randZSpeed(randomEngine), zombiePositions[i]);
	}

	// Set up the players guns
	const float BULLET_SPEED = 20.0f;
	_player->addGun(new Gun("Magnum", 50, 1, 0.2f, 50, BULLET_SPEED));
	_player->addGun(new Gun("Shotgun", 200, 4, 0.3f, 150, BULLET_SPEED));
	_player->addGun(new Gun("Mine", 100, 1, 0.0f, 150, 0.0f));
	_player->addGun(new Gun("Sniper Rifle", 50, 1, 0.001f, 300, 40.0f));
}
	...
	for (int i = 0; i < _zombies.size(); i++) {
		...
		for (int j = 1; j < _humans.size(); j++) {
			if (_zombies[i]->collideWithAgent(_humans[j])) {
				...
				std::uniform_real_distribution<float> randZSpeed(1.0f, 3.0f);
				_zombies.push_back(new Zombie);
				_zombies.back()->init(randZSpeed(randomEngine), _humans[j]->getPosition());
				// Delete the human
				delete _humans[j];
